...
train_df = loaded_dfs['train_df_low_samples_pendulum']
val_df = loaded_dfs['val_df_low_samples_pendulum']

train_id_df = loaded_dfs['train_id_df_low_samples_pendulum']
val_id_df = loaded_dfs['val_id_df_low_samples_pendulum']
...
val_df_high_energy = df_high_energy_low_samples_pendulum['val_df_high_energy_low_samples_pendulum']
val_id_df_high_energy = df_high_energy_low_samples_pendulum['val_id_df_high_energy_low_samples_pendulum']
...
def main():
    ...
    mapping_net = SimpleStackedHamiltonianNetwork(
        ...
        step_1_c1_mean_innit= 0.01,
        step_2_c1_mean_innit= -0.01,
        std_to_mean_ratio_c1_mean_init= 1.0,

        step_1_c2_mean_innit= -0.01,
        step_2_c2_mean_innit= 0.01,
    ...
    possible_t_values = get_data_from_trajectory_id(ids_df=train_id_df, data_df=train_df, trajectory_ids=0)['t'].values.tolist() #Same possible t values for every trajectory
    normalized_mse = False
    loss_type = "mae" #Either 'mae' or 'mse'
    predict_full_trajectory = True

    save_dir_1 = "./save_directory_low_samples_pendulum_1" 
    save_dir_2 = "./save_directory_noisy_low_samples_pendulum_2" 

    save_dir_3 = "./save_directory_noisy_full_pred_low_samples_pendulum_3"
    ...
    if not resume_training:
        ...
        train_model(
            ...
            prediction_coefficient=4.0,
            hsic_loss_max_want=0.3,
        ...
        checkpoint_path = os.path.join(save_dir_3, "best_model.pt")

        loss_scales_save_path = os.path.join(save_dir_3, "loss_scales.pkl")
        ...
        resume_training_from_checkpoint(
            ...
            # Optimizer parameters
            learning_rate=5e-5,  # MODE A: None=use checkpoint LR, or specify to override | MODE B: REQUIRED, must specify
            ...
            # Training parameters
            num_epochs=600,  # Number of ADDITIONAL epochs to train 
            ...
            # Checkpointing
            save_dir=save_dir_3,  # Should typically match the directory where checkpoint_path is located
            save_best_only=False,
            save_freq_epochs=10,
            auto_rescue=True,
...
def main():
    ...
    save_dir_4 = "./save_directory_noisy_full_pred_low_samples_pendulum_4"
    ...
    if not resume_training:
        ...
        train_model(
            ...
            scheduler_type='plateau', 
            scheduler_params={'mode': 'min', 'factor': 0.5, 'patience': 100, 'verbose': True},    # Dict of params specific to the scheduler. Set if it is a fresh training session.
        ...
        resume_training_from_checkpoint(
            ...
            # Scheduler parameters  
            scheduler_type='plateau',  # MODE A: must match original | MODE B: can be different
            scheduler_params={'mode': 'min', 'factor': 0.5, 'patience': 50, 'verbose': True},  # MODE A: can differ. Functionality would depend on reset_scheduler_patience | MODE B: can be different
            reset_scheduler_patience = True, #Only relevant on MODE A. Set True to reset num_bad_epochs. Use True if you want the learning rate to be lowered after the full patience amount. Use False if you want continuity, already waited N epochs, just need M-N more where M: loaded num_bad_epochs from previous training
